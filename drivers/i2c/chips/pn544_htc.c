...
int pn544_htc_check_rfskuid(int in_is_alive){
	...
	return 0;
#else 
	return pn544_htc_check_pid(in_is_alive);
#endif 
}

int pn544_htc_check_pid(int in_is_alive){

#ifdef NFC_READ_PID_B3_ASIA
	int nfc_pid_size = 0;
	unsigned int *nfc_pid_info;
	struct device_node *nfc_pid;
	nfc_pid = of_find_node_by_path("/chosen/board_info");
	if (nfc_pid){
		nfc_pid_info = (unsigned int *) of_get_property(nfc_pid,"pid",&nfc_pid_size);
		I("%s:The PID is %x\n",__func__,nfc_pid_info[0]);
		if(nfc_pid_info[0] == 0x81010000) return 0;
	}else {
		E("%s:Get pid fail keep NFC by default  \n",__func__);
		return 1;
	}
	return 1;
#else 
