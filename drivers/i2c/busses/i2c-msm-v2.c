				...
				bool use_param_vals, u32 status, u32 qup_op)
{
	...
	dev_err(ctrl->dev,"QUP_MX_OUTPUT_COUNT %x QUP_MX_INPUT_COUNT %x "
			  "QUP_MX_READ_COUNT %x QUP_MX_WRITE_COUNT %x,  "
			  "QUP_MX_OUTPUT_COUNT_CUR %x "
			  "QUP_MX_INPUT_COUNT_CUR %x QUP_MX_READ_COUNT_CUR %x "
			  "QUP_MX_WRITE_COUNT_CUR %x ",
			  readl_relaxed(base + QUP_MX_OUTPUT_COUNT),
			  readl_relaxed(base + QUP_MX_INPUT_COUNT),
			  readl_relaxed(base + QUP_MX_READ_COUNT),
			  readl_relaxed(base + QUP_MX_WRITE_COUNT),
			  readl_relaxed(base + 0x104),
			  readl_relaxed(base + 0x204),
			  readl_relaxed(base + 0x20C),
			  readl_relaxed(base + 0x154));

	if( xfer->msgs->addr == 0x72 ) {
		dev_err(ctrl->dev," GPIO state 23: %d 24: %d ",
				  gpio_get_value(ctrl->sda_gpio),
				  gpio_get_value(ctrl->scl_gpio));
	}
}
