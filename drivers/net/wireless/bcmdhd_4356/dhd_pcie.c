 ...
 *
 * $Id: dhd_pcie.c 550715 2015-04-21 10:34:56Z $
 */
...
dhdpcie_readshared(dhd_bus_t *bus)
{
	...
	
	{
		...
		if (bus->max_sub_queues > sizeof(bus->ring_sh)/sizeof(ring_sh_info_t)) {
			uint32 array_max_size = sizeof(bus->ring_sh)/sizeof(ring_sh_info_t);
			WARN(1, "%s: %u exceeds max allocated array size.\n",
					__FUNCTION__, array_max_size);
			bus->max_sub_queues = array_max_size;
		}
