 ...
 *
 * $Id: wl_android.c 544896 2015-03-30 09:46:52Z $
 */
...
#define PRIVATE_COMMAND_MAX_LEN	8192
	...
	if (strnicmp(command, CMD_STOP, strlen(CMD_STOP)) == 0) {
		...
		if (dhdp->op_mode & DHD_FLAG_HOSTAP_MODE) {
			...
			printf("%s Not in AP mode skip this %s cmd\n", __FUNCTION__, CMD_AP_MAC_LIST_SET);
		}
...
static int wl_android_set_ap_mac_list(struct net_device *dev, void *buf)
{
	...
	if (mac_mode == MACLIST_MODE_DISABLED) {
		...
		for (i = 0; i < white_maclist->count; i++) {
			if (white_maclist->count > 16) {
				DHD_ERROR(("invalid count white: %d\n", white_maclist->count));
				return 0;
			}

			DHD_ERROR(("mac %d: %02X:%02X:%02X:%02X:%02X:%02X\n", i,
		...
		for (i = 0; i < android_ap_black_list.count; i++) {
			if (android_ap_black_list.count > 16) {
				DHD_ERROR(("invalid count black: %d\n", android_ap_black_list.count));
				return 0;
			}
...
static int wl_android_allow_p2p_event(struct net_device *dev, char *command, int total_len)
{
        ...
        case 0:
                ...
                if (screen_off) {
                        ...
                        ret = wldev_iovar_getbuf(dev, "event_msgs", NULL, 0, eventmask, sizeof(eventmask), NULL);
                        if (ret) {
                                WL_ERR(("%s: wldev_iovar_getbuf() failed, ret=%d\n",
                                        __FUNCTION__, ret));
                                goto error;
                        }
                        bcm_print_bytes("eventmask", (uint8 *)eventmask, sizeof(eventmask));
                        ...
                        DHD_ERROR(("%s: screen is on, do nothing\n", __FUNCTION__));
                }
                break;
        case 1:
                dhdp->allow_p2p_event = set_val;
