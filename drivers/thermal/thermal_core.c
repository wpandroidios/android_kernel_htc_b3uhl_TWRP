...
static char *kddi_cool[2]    = { "KDDI_OVERHEAT=COOL", NULL };
static char *kddi_callsync[2]    = { "KDDI_OVERHEAT=KDDI", NULL };
static char *kddi_overheat[2]    = { "KDDI_OVERHEAT=OVERHEAT", NULL };
static char *kddi_shutdown[2]    = { "KDDI_OVERHEAT=SHUTDOWN", NULL };
...
static ssize_t
temp_notify_kddi_store(struct device *dev, struct device_attribute *attr,
          const char *buf, size_t count)
{
	struct thermal_zone_device *tz = to_thermal_zone(dev);
	unsigned int status=0;
	...
	if (strict_strtoul(buf, 10, &value))
		return -EINVAL;
	if (value == 0)
		status = kobject_uevent_env(&tz->device.kobj, KOBJ_CHANGE, kddi_cool);
	else if (value == 1)
		status = kobject_uevent_env(&tz->device.kobj, KOBJ_CHANGE, kddi_callsync);
	else if (value == 2)
		status = kobject_uevent_env(&tz->device.kobj, KOBJ_CHANGE, kddi_overheat);
	else if (value == 3)
		status = kobject_uevent_env(&tz->device.kobj, KOBJ_CHANGE, kddi_shutdown);
	else
		status = kobject_uevent_env(&tz->device.kobj, KOBJ_CHANGE, kddi_cool);

	pr_info("device temp kddi uevent :action=%s,sucess=%d\n",buf,status);
...
static DEVICE_ATTR(temp_notify_kddi, S_IWUSR, NULL, temp_notify_kddi_store);
	...
	int passive_delay, int polling_delay)
{
	...
	if (result)
        ...
        result = device_create_file(&tz->device, &dev_attr_temp_notify_kddi);
        if (result)
                goto unregister;
