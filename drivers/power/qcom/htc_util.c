...
#define SHARE_SCREEN_KERNEL_TOP_CPU_USAGE_THRESHOLD 5
...
static void htc_kernel_top_show(struct _htc_kernel_top *ktop, int type)
{
	...
	for (i = 0; i < NUM_BUSY_THREAD_CHECK; i++) {
		if (ktop->cpustat_time > 0) {
			...
			if ((top_n_pid == 0) && (ktop->curr_proc_delta[top_n_pid] == 0))
				continue;
			if (ktop->task_ptr_array[top_n_pid]->comm) {
				if (strcmp(ktop->task_ptr_array[top_n_pid]->comm, "i.android.evcsf") &&
					strcmp(ktop->task_ptr_array[top_n_pid]->comm, "ddi.android.evc"))
					continue;
				else
					kddi_cpu_usage += (ktop->curr_proc_delta[top_n_pid] * 100 / ktop->cpustat_time);
			}
		}
	}
                if (kddi_cpu_usage > SHARE_SCREEN_KERNEL_TOP_CPU_USAGE_THRESHOLD) {
                        pr_err("Notify PnP enable call sync policy");
			            set_call_sync("call_sync");
		}
                else if (kddi_cpu_usage == 0) {
                        pr_err("Notify PnP disable call sync policy");
			            set_call_sync("");
		}
