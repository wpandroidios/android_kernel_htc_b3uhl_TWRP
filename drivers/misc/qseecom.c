...
static int qseecom_load_app(struct qseecom_dev_handle *data, void __user *argp)
{
	...
	if (app_id) {
		...
		pr_err("App (%s) does'nt exist, loading apps for first time\n",
				...
				bool app_crash)
{
	...
	pr_err("%d: qseecom_unload_app is called\n", __LINE__);
	pr_err("%d: app_crash = %d, app_name = %s, app_id = %d\n",
		__LINE__, app_crash, data->client.app_name, data->client.app_id);
	...
	if (found_app) {
		...
		if (app_crash) {
			ptr_app->ref_cnt = 0;
			pr_err("app_crash: ref_count = 0\n");
		} else {
			pr_err("app not crash: ref_count = %d\n", ptr_app->ref_cnt);

			if (ptr_app->ref_cnt == 1) {
				ptr_app->ref_cnt = 0;
				pr_err("ref_count set to 0\n");
			} else {
				ptr_app->ref_cnt--;
				pr_err("Can't unload app(%d) inuse\n",
					ptr_app->app_id);
			}
		}
		if (unload) {
						...
						char *app_name, uint32_t size)
{
	...
	pr_err("%d: qseecom_start_app: %s is called\n", __LINE__, app_name);
	...
	*handle = kzalloc(sizeof(struct qseecom_handle), GFP_KERNEL);
	...
	if (ret > 0) {
		pr_err("App id %d for [%s] app exists\n", ret,
		...
		spin_unlock_irqrestore(
				&qseecom.registered_app_list_lock, flags);
		if (!found_app)
			pr_err("App_id %d [%s] was loaded but not registered\n",
					ret, (char *)app_ireq.app_name);
	} else {
		
		pr_err("%s: Loading app for the first time'\n",
				qseecom.pdev->init_name);
...
int qseecom_shutdown_app(struct qseecom_handle **handle)
{
	...
	pr_err("%d: qseecom_shutdown_app is called\n", __LINE__);
	...
	pr_err("%d: exit qseecom_shutdown_app\n", __LINE__);
	return ret;
}
...
long qseecom_ioctl(struct file *file, unsigned cmd, unsigned long arg)
{
	...
	case QSEECOM_IOCTL_UNLOAD_APP_REQ: {
		...
		mutex_lock(&app_access_lock);
		atomic_inc(&data->ioctl_count);
		pr_err("%d: userspace client call to unload app\n", __LINE__);
		ret = qseecom_unload_app(data, false);
		atomic_dec(&data->ioctl_count);
		mutex_unlock(&app_access_lock);
		if (ret)
...
static int qseecom_release(struct inode *inode, struct file *file)
{
	...
	if (data->released == false) {
		...
		case QSEECOM_CLIENT_APP:
			pr_err("%d: qseecom_release is called\n", __LINE__);
